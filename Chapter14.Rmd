...
$$SSE = \sum_{}^{}\left( y_i - {\hat{y}}_i \right)^2$$

with
<center> $df_2 = n - p - 1$ </center>
...
The **Total Sum of Squares,** $SST$, is the total amount of
prediction error we would make if we used $\bar{y}$ (the mean of
$y_i$) to predict the observed sample data. Where is the $SST$ and its
$df$ in the ANOVA table?

-   The $SST = \sum_{}^{}\left( y_i - \bar{y} \right)^2$, and
    has degrees of freedom $df = n - 1$
    
<br>

The **Sum of Squares due to Regression,** $SSR$, is how much we
...
The **Mean Square Error,** $MSE$, is the estimate of the
variance of $\epsilon$. Remember $\epsilon?$ It is the random error term
in the Regression Model: $y = \beta_0 + \beta_1x + \epsilon$. Where
...
- First, you could use the Estimated Regression Equation to
  predict $y$ (in other words, predict $\hat{y}_i$
  at each $x_i$)

- Second, you could use the next best option, which is to just
  use the mean of $\bar{y}$ to predict $y$ at each
  $x_i$.
  This is also called the 'Intercept only model' because
  the equation of $y$ does not include any $x$
  variables, thus being only an intercept, and
...
<br>
<center> **Regression Statistics**
</center>

| | | **Details** |
|---|:-----:|------|
| **Multiple R** | $R_{xy}\ or\ R_{\hat{y}y} = \sqrt{R^2}$ | Correlation Coefficients |
| **R Squared** | $R^2 = \frac{SSR}{SST}$ | Coefficient of Determination |
| **Adjusted R Squared** | $R_A^2$ | $R_A^2 = 1 - [(1 - R^2)(\frac{n-1}{n-p-1})]$ |
| **Standard Error** | $s = \sqrt{MSE}$ | Standard Error of the Estimate, or Root Mean Square Error ($RMSE$) |
...
Show the calculation of the Standard Error of the Estimate, $s.$
Interpret this value.

<br> 
<br> 

Show the calculation of the Correlation Coefficient, $R_{xy}.$
**Interpret this value.**
